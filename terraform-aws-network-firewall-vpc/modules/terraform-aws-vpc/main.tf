  ...
  tags = merge(
    {
      "Name" = var.single_nat_gateway ? "${var.name}-${var.public_subnet_suffix}" : format(
        "${var.name}-${var.public_subnet_suffix}-%s-RT",
...
resource "aws_route_table" "private" {
  ...
  count = local.create_vpc && local.max_subnet_length > 0 ? (var.enable_nat_gateway ? local.nat_gateway_count : 1) : 0
  ...
  tags = merge(
    {
      "Name" = var.single_nat_gateway ? "${var.name}-${var.private_subnet_suffix}" : format(
        "${var.name}-${var.private_subnet_suffix}-%s-RT",
        element(var.azs, count.index),
      )
    },
    var.tags,
...
################################################################################
# Intra routes
# There are as many routing tables as the number of CIDR Intra Subnets
################################################################################

resource "aws_route_table" "intra" {
  # count = local.create_vpc && length(var.intra_subnets) > 0 ? local.nat_gateway_count : 0
  ...
  tags = merge(
    {
      "Name" = var.single_nat_gateway ? "${var.name}-${var.intra_subnet_suffix}" : format(
        "${var.name}-${var.intra_subnet_suffix}-%s-RT",
        element(var.azs, count.index),
      )
    },
    var.tags,
...
################################################################################
# Firewall routes
# There are as many routing tables as the number of CIDR Firewall Subnets
################################################################################

resource "aws_route_table" "firewall" {
  count = local.create_vpc && length(var.firewall_subnets) > 0 ? local.nat_gateway_count : 0
  ...
  tags = merge(
    {
      "Name" = var.single_nat_gateway ? "${var.name}-${var.firewall_subnet_suffix}" : format(
        "${var.name}-${var.firewall_subnet_suffix}-%s-RT",
        element(var.azs, count.index),
      )
    },
    var.tags,
...
################################################################################
# Ffirewall subnets - private subnet without NAT gateway
################################################################################
...
resource "aws_network_acl" "public" {
  ...
  tags = merge(
    { "Name" = "${var.name}-${var.public_subnet_suffix}-NACL" },
    var.tags,
    var.public_acl_tags,
  )
}
...
resource "aws_network_acl" "private" {
  ...
  tags = merge(
    { "Name" = "${var.name}-${var.private_subnet_suffix}-NACL" },
    var.tags,
...
resource "aws_network_acl" "intra" {
  ...
  tags = merge(
    { "Name" = "${var.name}-${var.intra_subnet_suffix}-NACL" },
    var.tags,
    var.intra_acl_tags,
  )
}
...
resource "aws_route_table_association" "firewall" {
  ...
  # length(var.firewall_subnets) ? 0 : count.index, )
