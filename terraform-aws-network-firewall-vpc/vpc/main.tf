locals {
  availability_zones_names = slice(data.aws_availability_zones.available.names, 0, max(var.public_subnet_count, var.private_subnet_count, var.database_subnet_count, length(var.public_subnet_cidrs), length(var.private_subnet_cidrs), length(var.database_subnet_cidrs)))
  availability_zones_ids   = slice(data.aws_availability_zones.available.zone_ids, 0, max(var.public_subnet_count, var.private_subnet_count, var.database_subnet_count, length(var.public_subnet_cidrs), length(var.private_subnet_cidrs), length(var.database_subnet_cidrs)))
  ...
  firewall_subnets = length(var.firewall_subnet_cidrs) != 0 ? var.firewall_subnet_cidrs : [for index in range(var.firewall_subnet_count) : cidrsubnet(var.vpc_cidr, 8, index + var.firewall_subnet_network_start)]
}
...
module "vpc" {
  ...
  firewall_subnet_suffix    = var.firewall_subnet_suffix
  firewall_subnet_names     = [for az_name in local.availability_zones_names : "${var.vpc_name}-${var.firewall_subnet_suffix}-${az_name}-firewall-subnet"]
