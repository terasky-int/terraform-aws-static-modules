...
################################################################################
# Intra routes
# There are as many routing tables as the number of NAT gateways                    # Changed by Andrey
################################################################################

resource "aws_route_table" "intra" {
  count = local.create_vpc && length(var.intra_subnets) > 0 ? length(var.intra_subnets) : 0 # count = local.create_vpc && length(var.intra_subnets) > 0 ? local.nat_gateway_count : 0
...
resource "aws_route" "private_nat_gateway" {
  count = local.create_vpc && var.enable_nat_gateway && var.create_inspection_routing == false ? local.nat_gateway_count : 0
